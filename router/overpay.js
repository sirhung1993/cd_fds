...
const OverPayCheck = require('../control/OverPayCheck')

const suscessResponse = config.suscessResponse 
router.get('/all', async (req, res, next) => {
    const allSusOrder = await OverPayCheck.getAllSuspicionOrders()
...
router.get('/detail/:orderCode', async (req, res, next) => {
    ...
    const DwId = await OverPayCheck.getFdsSuspicionDetail(orderCode)
    if(!DwId.error) {
...
router.get('/:orderCode', async (req, res, next) => {
    const orderCode = req.params.orderCode
    //const page = parseInt(req.params.page) || 0
    const DwOrderCode = await OverPayCheck.getDwSuspicionOrders(orderCode)
    const DwId = await OverPayCheck.getFdsSuspicionOrders(orderCode)
...
router.get('/:limit/:page', async (req, res, next) => {
    ...
    const page = parseInt(req.params.page) || 0
    const fdsOrder = await OverPayCheck.getFdsSuspicionOrders(limit, page)
    if(!fdsOrder.error) {
